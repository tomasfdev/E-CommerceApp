...
export class BasketService {
  ...
  private addOrUpdateItem(items: BasketItem[], itemToAdd: BasketItem, quantity: number): BasketItem[] {
    ...
    if (item) item.quantity += quantity;  //se já existir um item igual itemToAdd, ñ adiciona itemToAdd ao BasketItem[], apenas aumenta a quantidade do item já existente no BasketItem[]
    else {  //se ñ existir o itemToAdd dentro do BasketItem[], adiciona itemToAdd ao BasketItem[]
  ...
  private isProduct(item: Product | BasketItem): item is Product {  //retorna um "Product" caso seja/retorne true
  ...
  private calculateTotals() {
    ...
    if (!basket) return;
    const shipping = 0;
    const subtotal = basket.items.reduce((totalItemsPrice, ItemPrice) => (ItemPrice.price * ItemPrice.quantity) + totalItemsPrice, 0); //multiplica o preço do item pela sua quantidade e soma ao total
